...
namespace FantasyDataAPI\Test\PlayerGameProjectionStatsByPlayerID\Response;
...
class Mock extends Response
{
    ...
    public function __construct (RequestInterface $pRequest)
    {
        ...
        list(, , , $format, , $season, $week, $playerID) = explode( '/', $pRequest->getPath() );
        /** hardcode subscription so we can keep using the same filenames!*/
        $subscription = 'developer';

        $file_partial = __DIR__ . '/' . implode('.', [$subscription, $format, $season, $week, $playerID]);
