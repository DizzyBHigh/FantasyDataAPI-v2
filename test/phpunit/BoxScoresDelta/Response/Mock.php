...
namespace FantasyDataAPI\Test\BoxScoresDelta\Response;
...
class Mock extends Response
{
    ...
    public function __construct (RequestInterface $pRequest)
    {
        /** url parsing "formula" for resource
            http://api.nfldata.apiphany.com/nfl/v2/JSON/BoxScores/2013REG/17/60
         */
        list(, , ,$format,  , $season, $week, $minutes) = explode( '/', $pRequest->getPath() );
        /**  hard code subscription so we use same filenames!  */
        $subscription = 'developer';

        $file_partial = __DIR__ . '/' . implode('.', [$subscription, $format, $season, $week, $minutes]);
